...
import {
  ...
  createProgressRing, createTextArea, openDialog, renderFractionsInText, renderRichText, escapeHtml,
  createSelectField
} from '../core/ui.js';
...
import { getOrCreateScratchPad } from './scratchpad.js';
...
export async function renderExamSession(container, categoryId = null) {
  ...
  function playExamGameplay(questions, limitMinutes) {
    ...
    function renderActiveQuestion() {
      ...
      leftBox.style.cssText = 'display:flex; align-items:center; gap:var(--space-2);';

      const scratchBtn = document.createElement('button');
      scratchBtn.className = 'icon-btn';
      scratchBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>';
      scratchBtn.style.color = isScratchPadOpen ? 'var(--color-primary)' : 'var(--text-secondary)';
      if (isScratchPadOpen) scratchBtn.style.background = 'rgba(108, 92, 231, 0.1)';
      scratchBtn.onclick = () => {
        isScratchPadOpen = !isScratchPadOpen;
        renderActiveQuestion();
      };
      leftBox.appendChild(scratchBtn);
      ...
      if (isScratchPadOpen) {
        const pad = getOrCreateScratchPad(() => {
      ...
      qPromptEl.innerHTML = renderRichText(q.prompt);
      ...
      if (q.type === 'choice') {
        q.options.forEach((opt, oIdx) => {
          ...
          btn.innerHTML = `<span style="flex:1;">${renderRichText(opt)}</span><span style="width:20px; height:20px; border-radius:50%; border:2px solid var(--border-strong); display:flex; align-items:center; justify-content:center; flex-shrink:0;"><span style="width:10px; height:10px; border-radius:50%; background:transparent;"></span></span>`;
