...
export async function renderAI(container) {
  ...
  sidebar.style.cssText = 'position:fixed; top:0; bottom:0; left:0; width:300px; max-width:80vw; z-index:999; background:color-mix(in srgb, var(--bg-card) 85%, transparent); backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px); transform:translateX(-100%); transition:transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 4px 0 24px rgba(0,0,0,0.1); display:flex; flex-direction:column; padding:calc(var(--space-3) + env(safe-area-inset-top, 0px)) var(--space-3) var(--space-3) var(--space-3); gap:var(--space-3);';
  ...
  function renderInteractiveStep() {
    ...
    actionRow.style.cssText = 'display:flex; justify-content:space-between; align-items:center; gap:8px; flex-shrink:0;';
    ...
    interactivePanelContainer.appendChild(actionRow);
  }
...
const PADDING = spec.charge != null ? 65 : 50; // extra padding for brackets
  ...
  function initLewisStructures(parent) {
    const cards = parent.querySelectorAll('.interactive-lewis-card');
    cards.forEach((card) => {
      const btn = card.querySelector('.lewis-download-btn');
      const svg = card.querySelector('.lewis-svg');
      if (!btn || !svg) return;
      btn.addEventListener('click', () => {
        downloadSvgAsPng(svg, btn.getAttribute('data-filename') || 'lewis-structure');
      });
    });
  }
  ...
  function renderMarkdownAndMath(text) {
    ...
    html = html.replace(/\$\$\s*([\s\S]*?)\s*\$\$/g, (match, math) => {
      ...
      mathBlocks.push(`<div class="math-block" style="text-align:center; padding:var(--space-2) var(--space-3); margin:var(--space-2) 0; background:var(--bg-sunken); border:1px solid var(--border-subtle); border-radius:10px; font-style:italic; font-family:var(--font-mono); font-size:15px; direction:ltr; display:block; overflow-x:auto; white-space:nowrap; -webkit-overflow-scrolling:touch; cursor:zoom-in; box-shadow:inset 0 1px 2px rgba(0,0,0,0.02);">${cleanMath}</div>`);
      return placeholder;
    });
    ...
    html = html.replace(/\$\s*([^$]+?)\s*\$/g, (match, math) => {
      ...
      mathInlines.push(`<span class="math-inline" style="font-family:var(--font-mono); font-style:italic; background:var(--bg-sunken); padding:2px 6px; border-radius:6px; font-weight:700; color:var(--color-primary); font-size:0.95em; direction:ltr; display:inline-block; max-width:100%; overflow-x:auto; white-space:nowrap; vertical-align:middle; -webkit-overflow-scrolling:touch; cursor:zoom-in; border:1px solid var(--border-subtle);">${cleanMath}</span>`);
      return placeholder;
    });
    ...
    for (let i = 0; i < lewisStructures.length; i++) {
      const spec = parseLewisSpec(lewisStructures[i]);
      const { svg } = buildLewisSvg(spec);
      const lewisHtml = `
        <div class="interactive-lewis-card" style="
          background: var(--bg-card);
          border: 1px solid var(--border-soft);
          border-radius: 16px;
          padding: var(--space-4);
          ...
          direction: rtl;
        ">
          <div style="display: flex; align-items: center; gap: 6px; font-weight: 800; color: var(--color-primary); font-size: var(--text-body);">
            <span class="material-symbols-rounded" style="color: var(--color-primary);">science</span>
            <span>${escapeHtml(spec.title)}</span>
          </div>
          <div style="width:100%; overflow-x:auto; display:flex; justify-content:center; background: var(--bg-sunken); border-radius: 12px; border: 1px solid var(--border-subtle); padding: var(--space-3);">
            ${svg}
          </div>
          <button class="lewis-download-btn btn btn-outline" style="align-self:flex-end; font-size:12px; padding:6px 12px;" data-filename="${escapeHtml(spec.title || 'lewis-structure')}">
            <span class="material-symbols-rounded" style="font-size:16px;">download</span> دانلود عکس ساختار
          </button>
        </div>
      `;
  ...
  function buildAskQuestionWidget(aq) {
    ...
    header.innerHTML = '<span class="material-symbols-rounded" style="font-size:18px; color:var(--color-primary);">psychology_alt</span><span>سوال تمرینی — پاسخت را اینجا بنویس</span>';
    ...
    async function runCheck() {
      ...
      if (grade.correct) {
        ...
        explanationBox.textContent = grade.explanation || '';
      } else {
        ...
        explanationBox.textContent = grade.explanation || 'توضیحی دریافت نشد.';
  ...
  function renderMessage(sender, text, attachments = null, opts = {}) {
    ...
    if (qMatch) {
      ...
      try {
        askQuestion = JSON.parse(qMatch[1].trim());
      } catch (e) {
        try {
          askQuestion = JSON.parse(fixJsonEscape(qMatch[1].trim()));
        } catch (e2) {
          console.error('Failed to parse ask-question JSON:', e, e2);
        }
      }
      if (askQuestion && !askQuestion.question) askQuestion = null;
    }
