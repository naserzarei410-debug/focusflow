...
export async function renderHome(container) {
  ...
  if (streak.currentStreak > 0) {
    ...
    streakWidget.innerHTML = `
      ...
      <div style="display:flex; flex-direction:column; gap:2px; text-align:right;">
        <span style="font-size:var(--text-body); font-weight:800;">تداوم یادگیری شما عالی است.</span>
        <span style="font-size:var(--text-caption); font-weight:600; opacity:0.9;">${streak.currentStreak.toLocaleString('fa-IR')} روز مطالعه متوالی. ادامه دهید.</span>
      </div>
    `;
  ...
  if (categories.length > 0) {
    ...
    if (reviewedToday >= dailyGoal) {
      ...
      goalFooter.innerHTML = '<span class="material-symbols-rounded" style="font-size:18px; color:#10B981;">celebration</span> <span style="color:#10B981; font-weight:700;">هدف امروز محقق شد.</span>';
    } else {
  ...
  if (totalDue > 0 || totalNew > 0) {
    ...
    `;
    captionLabel.textContent = 'پیشرفت امروز';
...
async function startPdfEngineFlow(categories) {
  ...
  uploadBox.innerHTML = `
    ...
    <span style="font-size: 14px; font-weight: 700; color: var(--text-primary);">انتخاب یا رها کردن PDF</span>
  ...
  async function handleFileSelected() {
    ...
    loadingOverlay.innerHTML = `
      <div class="dialog-content" style="text-align: center; display: flex; flex-direction: column; align-items: center; gap: var(--space-3);">
        ...
        <h3 id="pdf-loading-text" style="font-size: 16px; font-weight: 700; color: var(--text-primary);">در حال استخراج...</h3>
        <p style="font-size: 13px; color: var(--text-secondary);">اطلاعات فقط در این دستگاه ذخیره می‌شود.</p>
      </div>
    `;
...
function openApprovalDialog(initialCards, categoryId) {
  ...
  function renderCardList() {
    content.innerHTML = '';
    if (cards.length === 0) {
      content.innerHTML = '<p style="text-align:center; color:var(--text-secondary); padding:var(--space-4);">کارت بررسی‌نشده‌ای وجود ندارد.</p>';
      return;
    }
    ...
    actions: [
      { label: 'لغو', variant: 'secondary' },
      {
        ...
        onClick: async () => {
          ...
          openDialog({
            title: 'موفقیت‌آمیز',
            content: `${cards.length.toLocaleString('fa-IR')} فلش‌کارت به دسته افزوده شد.`,
...
export async function renderAI(container) {
  ...
  let interactiveFormCollapsed = false;

  function cancelInteractiveFormFlow() {
    interactiveFormCollapsed = false;
    interactivePanelContainer.style.display = 'none';
    interactivePanelContainer.style.maxHeight = '60vh';
    interactivePanelContainer.style.padding = 'var(--space-4)';
    inputContainer.style.display = 'flex';
    interactiveFormFlow = null;
    interactiveFormAnswers = [];
    currentInteractiveStep = 0;
  }

  function setInteractiveFormCollapsed(collapsed) {
    interactiveFormCollapsed = !!collapsed;
    const body = interactivePanelContainer.querySelector('[data-iform-body]');
    const collapseBtn = interactivePanelContainer.querySelector('[data-iform-collapse]');
    if (body) body.style.display = interactiveFormCollapsed ? 'none' : 'flex';
    if (collapseBtn) {
      collapseBtn.innerHTML = interactiveFormCollapsed
        ? '<span class="material-symbols-rounded" style="font-size:20px;">keyboard_arrow_up</span>'
        : '<span class="material-symbols-rounded" style="font-size:20px;">keyboard_arrow_down</span>';
      collapseBtn.title = interactiveFormCollapsed ? 'باز کردن فرم' : 'جمع کردن فرم';
      collapseBtn.setAttribute('aria-label', collapseBtn.title);
    }
    interactivePanelContainer.style.maxHeight = interactiveFormCollapsed ? '72px' : '60vh';
    interactivePanelContainer.style.padding = interactiveFormCollapsed ? '10px 14px' : 'var(--space-4)';
  }

  function startInteractiveFormFlow(formJson) {
    if (!formJson || !formJson.steps || formJson.steps.length === 0) return;
    interactiveFormFlow = formJson;
    interactiveFormAnswers = formJson.steps.map(s => ({ question: s.question, options: [], text: '' }));
    currentInteractiveStep = 0;
    interactiveFormCollapsed = false;
    
    inputContainer.style.display = 'none';
    interactivePanelContainer.style.display = 'flex';
    interactivePanelContainer.style.maxHeight = '60vh';
    interactivePanelContainer.style.padding = 'var(--space-4)';
  ...
  function renderInteractiveStep() {
    ...
    const step = interactiveFormFlow.steps[currentInteractiveStep];
    const totalSteps = interactiveFormFlow.steps.length;
    
    const stepAns = interactiveFormAnswers[currentInteractiveStep];

    // Top toolbar: collapse (fold down so AI text is readable) + cancel
    const toolBar = document.createElement('div');
    toolBar.style.cssText = 'display:flex; align-items:center; justify-content:space-between; gap:8px; flex-shrink:0; margin-bottom:4px;';
    const toolTitle = document.createElement('span');
    toolTitle.style.cssText = 'font-size:12px; font-weight:800; color:var(--color-primary); display:flex; align-items:center; gap:6px;';
    toolTitle.innerHTML = '<span class="material-symbols-rounded" style="font-size:18px;">edit_note</span><span>فرم پاسخ</span>';
    const toolBtns = document.createElement('div');
    toolBtns.style.cssText = 'display:flex; align-items:center; gap:4px;';
    const collapseBtn = document.createElement('button');
    collapseBtn.type = 'button';
    collapseBtn.setAttribute('data-iform-collapse', '1');
    collapseBtn.title = 'جمع کردن فرم';
    collapseBtn.setAttribute('aria-label', 'جمع کردن فرم');
    collapseBtn.style.cssText = 'width:36px; height:36px; border-radius:10px; border:1px solid var(--border-soft); background:var(--bg-sunken); color:var(--text-secondary); display:flex; align-items:center; justify-content:center; cursor:pointer; padding:0;';
    collapseBtn.innerHTML = '<span class="material-symbols-rounded" style="font-size:20px;">keyboard_arrow_down</span>';
    collapseBtn.addEventListener('click', () => setInteractiveFormCollapsed(!interactiveFormCollapsed));
    const cancelBtn = document.createElement('button');
    cancelBtn.type = 'button';
    cancelBtn.title = 'لغو فرم';
    cancelBtn.setAttribute('aria-label', 'لغو فرم');
    cancelBtn.style.cssText = 'width:36px; height:36px; border-radius:10px; border:1px solid var(--border-soft); background:var(--bg-sunken); color:var(--color-danger); display:flex; align-items:center; justify-content:center; cursor:pointer; padding:0;';
    cancelBtn.innerHTML = '<span class="material-symbols-rounded" style="font-size:20px;">close</span>';
    cancelBtn.addEventListener('click', () => cancelInteractiveFormFlow());
    toolBtns.append(collapseBtn, cancelBtn);
    toolBar.append(toolTitle, toolBtns);
    interactivePanelContainer.appendChild(toolBar);
    
    const stepWrapper = document.createElement('div');
    stepWrapper.setAttribute('data-iform-body', '1');
    stepWrapper.style.cssText = 'display:flex; flex-direction:column; flex:1; overflow-x:hidden; overflow-y:auto; min-height:0;';
    
    let touchStartX = 0;
    stepWrapper.addEventListener('touchstart', (e) => {
      touchStartX = e.changedTouches[0].screenX;
    }, {passive: true});
    stepWrapper.addEventListener('touchend', (e) => {
      const touchEndX = e.changedTouches[0].screenX;
      if (touchEndX < touchStartX - 50 && currentInteractiveStep > 0) {
        saveCurrentStateAndGo(currentInteractiveStep - 1);
      } else if (touchEndX > touchStartX + 50 && currentInteractiveStep < totalSteps - 1) {
        saveCurrentStateAndGo(currentInteractiveStep + 1);
      }
    });

    function saveCurrentStateAndGo(nextIndex) {
      if (customTextInput) stepAns.text = customTextInput.value.trim();
      stepAns.options = Array.from(selectedOptions);
      if (nextIndex !== currentInteractiveStep) {
        currentInteractiveStep = nextIndex;
        renderInteractiveStep();
      }
    }
    ...
    questionText.style.cssText = 'font-size:16px; font-weight:800; color:var(--text-primary); margin:0; line-height:1.5;';
    questionText.innerHTML = renderMarkdownAndMath(step.question || step.description || 'سوال');
    initMathZoom(questionText);
    
    headerRow.appendChild(questionText);
    stepWrapper.appendChild(headerRow);
    
    if (step.description) {
      const descText = document.createElement('p');
      descText.style.cssText = 'font-size:13px; color:var(--text-secondary); margin:4px 0 12px 0; line-height:1.5;';
      descText.textContent = step.description;
      stepWrapper.appendChild(descText);
    } else {
      questionText.style.marginBottom = '12px';
    }
    
    const selectedOptions = new Set(stepAns.options || []);
    const isMulti = step.selectionType === 'multi';
    
    const optionsContainer = document.createElement('div');
    optionsContainer.style.cssText = 'display:flex; flex-direction:column; gap:8px; margin-bottom:16px; flex-shrink:0;';
    
    if (step.options && step.options.length > 0) {
      step.options.forEach(opt => {
        const optBtn = document.createElement('button');
        const isSelected = selectedOptions.has(opt);
        optBtn.style.cssText = `text-align:right; padding:12px 16px; border-radius:12px; font-size:14px; font-weight:600; border:1px solid ${isSelected ? 'var(--color-primary)' : 'var(--border-soft)'}; background:${isSelected ? 'var(--color-primary-soft)' : 'var(--bg-secondary)'}; color:var(--text-primary); cursor:pointer; transition:all 0.2s ease; display:flex; align-items:center; gap:12px; flex-shrink:0;`;
        
        const checkCircle = document.createElement('span');
        checkCircle.className = 'material-symbols-rounded';
        checkCircle.style.cssText = `font-size:20px; color:${isSelected ? 'var(--color-primary)' : 'var(--border-strong)'}; transition:all 0.2s;`;
        
        if (isMulti) {
           checkCircle.textContent = isSelected ? 'check_box' : 'check_box_outline_blank';
        } else {
           checkCircle.textContent = isSelected ? 'radio_button_checked' : 'radio_button_unchecked';
        }
        
        const optLabel = document.createElement('span');
        optLabel.textContent = opt;
        
        optBtn.append(checkCircle, optLabel);
        ...
        optBtn.addEventListener('click', () => {
          if (isMulti) {
            if (selectedOptions.has(opt)) {
              ...
              optBtn.style.borderColor = 'var(--border-soft)';
              checkCircle.textContent = 'check_box_outline_blank';
              checkCircle.style.color = 'var(--border-strong)';
            } else {
              selectedOptions.add(opt);
              optBtn.style.background = 'var(--color-primary-soft)';
              optBtn.style.borderColor = 'var(--color-primary)';
              checkCircle.textContent = 'check_box';
            ...
            Array.from(optionsContainer.children).forEach(child => {
               child.style.background = 'var(--bg-secondary)';
               child.style.borderColor = 'var(--border-soft)';
               child.firstChild.textContent = 'radio_button_unchecked';
               child.firstChild.style.color = 'var(--border-strong)';
            });
            optBtn.style.background = 'var(--color-primary-soft)';
            optBtn.style.borderColor = 'var(--color-primary)';
            checkCircle.textContent = 'radio_button_checked';
            checkCircle.style.color = 'var(--color-primary)';
          }
          stepAns.options = Array.from(selectedOptions);
        });
      ...
      stepWrapper.appendChild(optionsContainer);
    }
    
    let customTextInput = null;
    if (step.allowCustomText !== false || (!step.options || step.options.length === 0)) {
      customTextInput = document.createElement('textarea');
      customTextInput.rows = 2;
      customTextInput.placeholder = (step.options && step.options.length > 0) ? 'پاسخ دلخواه خود را اضافه کنید...' : 'پاسخ خود را بنویسید...';
      customTextInput.value = stepAns.text || '';
      customTextInput.style.cssText = 'width:100%; box-sizing:border-box; padding:12px; border-radius:12px; border:1px solid var(--border-soft); background:var(--bg-sunken); color:var(--text-primary); font-size:14px; resize:vertical; font-family:inherit; outline:none; transition:border-color 0.2s; margin-bottom:16px; flex-shrink: 0;';
      customTextInput.addEventListener('focus', () => customTextInput.style.borderColor = 'var(--color-primary)');
      customTextInput.addEventListener('blur', () => customTextInput.style.borderColor = 'var(--border-soft)');
      customTextInput.addEventListener('input', () => { stepAns.text = customTextInput.value; });
      stepWrapper.appendChild(customTextInput);
    }
    
    const actionRow = document.createElement('div');
    actionRow.style.cssText = 'display:flex; justify-content:space-between; align-items:center; gap:8px; flex-shrink:0; margin-top:8px;';
    
    const spacer = document.createElement('div');
    spacer.style.cssText = 'width:40px; flex-shrink:0;';
    
    const linesContainer = document.createElement('div');
    linesContainer.style.cssText = 'display:flex; gap:6px; flex:1; justify-content:center;';
    
    for (let i = 0; i < totalSteps; i++) {
       const line = document.createElement('div');
       line.style.cssText = `height:4px; width:20px; border-radius:2px; cursor:pointer; background: ${i === currentInteractiveStep ? 'var(--color-primary)' : 'var(--color-primary-soft)'}; transition:background 0.2s;`;
       line.addEventListener('click', () => {
         saveCurrentStateAndGo(i);
       });
       linesContainer.appendChild(line);
    }
    
    const submitBtn = document.createElement('button');
    const isLast = currentInteractiveStep === totalSteps - 1;
    submitBtn.className = 'btn btn-primary';
    submitBtn.style.cssText = 'width:40px; height:40px; border-radius:20px; display:flex; align-items:center; justify-content:center; padding:0; flex-shrink:0; box-shadow: 0 2px 8px color-mix(in srgb, var(--color-primary) 30%, transparent);';
    
    if (isLast) {
      submitBtn.innerHTML = SEND_ICON_SVG;
    } else {
      submitBtn.innerHTML = SEND_ICON_SVG.replace('<svg ', '<svg style="transform: rotate(-90deg);" ');
    }
    
    submitBtn.addEventListener('click', () => {
      saveCurrentStateAndGo(currentInteractiveStep);
      if (isLast) {
        finishInteractiveFormFlow();
      } else {
        saveCurrentStateAndGo(currentInteractiveStep + 1);
      }
    });
    
    actionRow.append(spacer, linesContainer, submitBtn);
    // Keep action row inside the collapsible body so fold hides everything but the toolbar
    stepWrapper.appendChild(actionRow);
    interactivePanelContainer.appendChild(stepWrapper);
    setInteractiveFormCollapsed(interactiveFormCollapsed);
  }

  function finishInteractiveFormFlow() {
    let finalPayload = '';
    interactiveFormAnswers.forEach((ans, idx) => {
      finalPayload += `مرحله ${idx + 1}: ${ans.question}\n`;
      if (ans.options.length > 0) finalPayload += `گزینه‌ها: ${ans.options.join(' | ')}\n`;
      if (ans.text) finalPayload += `پاسخ متنی: ${ans.text}\n`;
      finalPayload += '\n';
    });
    ...
    inputField.input.value = finalPayload.trim();
    adjustInputHeight();
    handleSend();
    ...
    for (const file of files) {
      ...
      if ((file.type || '').startsWith('image/')) {
        try {
          ...
          if (err && (err.message === 'cancelled' || String(err).includes('cancel'))) continue;
          processed = file;
        }
      }
  ...
  async function requestAIResponse() {
    ...
    try {
      ...
      renderMessage('ai', resData.text, null, { isLast: true });
      chatList.scrollTo({ top: chatList.scrollHeight, behavior: 'smooth' });
...
<svg class="ai-custom-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="width: 18px; height: 18px; display: inline-block; vertical-align: middle;">
  ...
  const saveGoalBtn = createButton({
    ...
    onClick: async () => {
      ...
      showStatusMessage(goalContainer, 'اهداف روزانه (کارت و تمرکز) ذخیره شد.', 'success');
    }
  });
  saveGoalBtn.style.alignSelf = 'flex-end';
  goalContainer.append(goalField, focusGoalField, saveGoalBtn);
