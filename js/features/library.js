...
export async function renderLibrary(container) {
  ...
  async function onRefresh() {
    ...
    for (const cat of activeCategories) {
      ...
      const titleEl = catCard.querySelector('h3');
      const descEl = catCard.querySelector('p');
      const avatar = document.createElement('div');
      avatar.style.cssText = 'width:44px; height:44px; border-radius:12px; flex-shrink:0; display:flex; align-items:center; justify-content:center; overflow:hidden; background:var(--bg-sunken);';
      if (cat.coverImage) {
        ...
        avatar.innerHTML = `<span class="material-symbols-rounded" style="font-size:24px; color:${cat.themeColor || '#3D6BFF'};">${cat.icon || 'folder'}</span>`;
      }
      const headerRow = document.createElement('div');
      headerRow.style.cssText = 'display:flex; align-items:center; gap:var(--space-3); margin-bottom:var(--space-2);';
      const textCol = document.createElement('div');
      textCol.style.cssText = 'flex:1; min-width:0;';
      if (titleEl) { titleEl.style.marginBottom = '2px'; textCol.appendChild(titleEl); }
      if (descEl) { descEl.style.marginBottom = '0'; textCol.appendChild(descEl); }
      headerRow.append(avatar, textCol);
      catCard.insertBefore(headerRow, catCard.firstChild);
      ...
      editBtn.className = 'material-symbols-rounded';
      editBtn.type = 'button';
      editBtn.title = 'ویرایش دسته';
      editBtn.textContent = 'edit';
      editBtn.style.cssText = `
        position:absolute; top:var(--space-2); left:var(--space-2);
        width:24px; height:24px; border-radius:50%; border:none;
        background:transparent; color:var(--text-secondary);
        display:${selectionMode ? 'none' : 'flex'}; align-items:center; justify-content:center;
        font-size:17px; cursor:pointer; z-index:5; opacity:0.55;
      `;
      editBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        openEditCategoryDialog(cat, onRefresh);
      });
      catCard.appendChild(editBtn);
...
function openEditCategoryDialog(cat, onSuccess) {
  let title = cat.title || '';
  let description = cat.description || '';
  let themeColor = cat.themeColor || '#3D6BFF';
  let coverImage = cat.coverImage || null;
  ...
  imageLabel.textContent = 'آیکون دسته (اختیاری)';
  form.appendChild(imageLabel);

  const imageRow = document.createElement('div');
  imageRow.style.cssText = 'display:flex; align-items:center; gap:var(--space-3);';

  const preview = document.createElement('div');
  preview.style.cssText = 'width:56px; height:56px; border-radius:14px; overflow:hidden; background:var(--bg-sunken); display:flex; align-items:center; justify-content:center; flex-shrink:0;';
  function renderPreview() {
    preview.innerHTML = coverImage
      ? `<img src="${coverImage}" style="width:100%;height:100%;object-fit:cover;">`
      : `<span class="material-symbols-rounded" style="font-size:28px; color:var(--text-secondary);">folder</span>`;
  }
  renderPreview();
  ...
  const pickBtn = createButton({ label: 'انتخاب عکس', variant: 'secondary', onClick: () => fileInput.click() });
  pickBtn.style.cssText += '; font-size:12px; padding:6px 10px;';
  const removeBtn = createButton({ label: 'استفاده از آیکون پیش‌فرض', variant: 'text', onClick: () => { coverImage = null; renderPreview(); } });
  removeBtn.style.cssText += '; font-size:12px; padding:6px 10px;';

  const btnCol = document.createElement('div');
  btnCol.style.cssText = 'display:flex; flex-direction:column; gap:6px; align-items:flex-start;';
  btnCol.append(pickBtn, removeBtn);

  imageRow.append(preview, btnCol, fileInput);
    ...
    actions: [
      ...
      },
      {
        ...
        onClick: async () => {
          ...
          await categoryRepository.update(cat.id, {
            title: title.trim(),
