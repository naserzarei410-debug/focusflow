...
export async function renderPracticeSession(container, categoryId = null) {
  ...
  function playPracticeGameplay(questions) {
    ...
    function renderQuestion() {
      ...
      promptText.innerHTML = renderRichText(q.prompt);
      ...
      if (q.type === 'choice') {
        q.options.forEach((opt, oIdx) => {
          ...
          optText.innerHTML = renderRichText(opt);
          ...
          optBtn.addEventListener('click', () => {
            ...
            if (isCorrect) {
              ...
              optBtn.style.background = 'var(--color-success-soft)';
              ...
              if (correctBtn) {
                ...
                correctBtn.style.background = 'var(--color-success-soft)';
        ...
        const choices = [
          { val: 'true', label: 'بله، صحیح است', icon: 'check_circle', color: 'var(--color-success)', bg: 'var(--color-success-soft)' },
        ...
        const subBtn = createButton({
          ...
          onClick: () => {
            ...
            if (isCorrect) {
              ...
              textInput.style.background = 'var(--color-success-soft)';
    ...
    async function renderFinalSummary() {
      ...
      const sessionRecord = {
        ...
        date: (() => { const d = new Date(startTime); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; })(),
        ...
        questions.forEach(q => {
          ...
          if (matchingHistory && !matchingHistory.isCorrect && !listedCards.includes(q.card.id)) {
            ...
            bkButton.innerHTML = q.card.bookmark 
              ? '<span class="material-symbols-rounded" style="font-size:12px">bookmark_fill</span><span>نشان‌شده</span>'
              : '<span class="material-symbols-rounded" style="font-size:12px">bookmark</span><span>نشان کردن</span>';
