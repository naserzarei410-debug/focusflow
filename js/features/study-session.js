...
import { createButton, createCard, createEmptyState, openDialog, createProgressBar, escapeHtml, escapeAttr, renderFractionsInText, renderRichText, showToast } from '../core/ui.js';
import { getStudyQueues, StudySession, calculateStreak, getRemainingNewCardQuota } from '../core/study.js';
import { Rating, State, schedule } from '../core/fsrs.js';
import { speak, isSpeechSupported } from '../core/tts.js';
import { rescheduleDueCardsReminder, refreshOngoingStatusNotification } from '../core/notifications.js';
...
/**
 * One-shot session-size cap, used by the home page's "2-minute mode"
 * quick-start button. It's set right before router.navigate('study') and
 * consumed (reset to null) the moment the next session actually starts,
 * so it never lingers and affect a later, normal full review.
 */
let nextSessionLimit = null;
export function setNextSessionLimit(limit) {
  nextSessionLimit = limit;
}

/**
 * Optional multi-category filter for the next study session.
 * Set right before router.navigate('study') from the home picker;
 * consumed (and cleared) when the session starts.
 * null / empty => all categories (global session).
 */
let nextSessionCategoryIds = null;
export function setNextSessionCategoryIds(ids) {
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    nextSessionCategoryIds = null;
    return;
  }
  nextSessionCategoryIds = Array.from(new Set(ids.map(String).filter(Boolean)));
}
...
export async function renderStudySession(container, categoryId = null, forceAll = false) {
  ...
  // Consume multi-category filter set by the home page picker (one-shot).
  let multiCategoryIds = null;
  if (nextSessionCategoryIds && nextSessionCategoryIds.length) {
    multiCategoryIds = [...nextSessionCategoryIds];
    nextSessionCategoryIds = null;
    // Single selection collapses to the existing single-category path.
    if (multiCategoryIds.length === 1 && !categoryId) {
      categoryId = multiCategoryIds[0];
      multiCategoryIds = null;
    }
  }
  ...
  if (multiCategoryIds && multiCategoryIds.length > 1) {
    const merged = { due: [], learning: [], new: [] };
    for (const id of multiCategoryIds) {
      const q = await getStudyQueues(id);
      merged.due.push(...(q.due || []));
      merged.learning.push(...(q.learning || []));
      merged.new.push(...(q.new || []));
      newTotalAvailable += q.newTotalAvailable || 0;
    }
    // Each category's getStudyQueues() call above already capped its own
    // "new" list against the same global daily quota independently, so
    // combining several categories can still add up to more new cards than
    // the daily allowance. Trim the merged list back down to that shared
    // allowance here.
    const sharedQuota = await getRemainingNewCardQuota();
    merged.new = merged.new.slice(0, sharedQuota);
    queues = merged;
    sessionCards = [...queues.due, ...queues.learning, ...queues.new];
  } else {
    queues = await getStudyQueues(categoryId);
    sessionCards = [...queues.due, ...queues.learning, ...queues.new];
    newTotalAvailable = queues.newTotalAvailable || 0;
  }

  if (forceAll) {
    if (multiCategoryIds && multiCategoryIds.length > 1) {
      const idSet = new Set(multiCategoryIds);
      const allCards = await flashcardRepository.getAll();
      sessionCards = allCards
        .filter((c) => !c.deleted && idSet.has(String(c.categoryId)))
  ...
  // Consume the one-shot "2-minute mode" cap, if the home page's quick-start
  // button set one right before navigating here. Never applies to a
  // "review again" (forceAll) session.
  if (!forceAll && nextSessionLimit) {
    sessionCards = sessionCards.slice(0, nextSessionLimit);
  }
  ...
  if (sessionCards.length === 0) {
    ...
    if (!forceAll) {
      if (multiCategoryIds && multiCategoryIds.length > 1) {
        const idSet = new Set(multiCategoryIds.map(String));
        const allCards = await flashcardRepository.getAll();
        hasAnyCards = allCards.some((c) => !c.deleted && idSet.has(String(c.categoryId)));
      } else {
        const allCards = categoryId
          ? await flashcardRepository.getByIndex('categoryId', categoryId)
          : await flashcardRepository.getAll();
    ...
    if (hasAnyCards) {
      actionsWrap.appendChild(createButton({
        ...
        onClick: () => {
          if (multiCategoryIds && multiCategoryIds.length > 1) {
            setNextSessionCategoryIds(multiCategoryIds);
          }
          renderStudySession(container, categoryId, true);
        }
      }));
    }
    actionsWrap.appendChild(createButton({
    ...
    const multiLabel = multiCategoryIds && multiCategoryIds.length > 1
      ? `${multiCategoryIds.length.toLocaleString('fa-IR')} دستهٔ انتخاب‌شده`
      : null;

    // An empty queue can mean the daily new-card limit was reached rather
    // than there being genuinely nothing left — those need different copy.
    const quotaLimited = !forceAll && newTotalAvailable > 0;

    if (quotaLimited) {
      container.appendChild(
        createEmptyState({
          icon: 'history_toggle_off',
          title: 'سقف کارت جدید امروز پر شده',
          desc: 'کارت‌های موعددار امروز تمام شده و سقف کارت‌های جدید هم پر شده. کارت‌های جدید بیشتری هست که فردا نشان داده می‌شوند — یا از تنظیمات، سقف روزانه را افزایش دهید.',
          action: actionsWrap
        })
      );
    } else if (hasAnyCards) {
      container.appendChild(
        createEmptyState({
          ...
          desc: category 
            ...
            : multiLabel
              ? `فلش‌کارت‌های ${multiLabel} کاملاً به‌روز هستند.`
              : 'همه فلش‌کارت‌های فعال شما کاملاً به‌روز هستند!',
          action: actionsWrap
        })
      );
    } else {
      container.appendChild(
        createEmptyState({
          ...
          desc: category 
            ? `در دسته‌ی «${category.title}» هنوز هیچ فلش‌کارتی نساخته‌اید.`
            : multiLabel
              ? `در ${multiLabel} هنوز فلش‌کارتی نیست.`
              : 'شما هنوز هیچ فلش‌کارتی نساخته‌اید.',
          action: actionsWrap
        })
      );
    }
  ...
  // Multi-category sessions are recorded without a single categoryId
  // so per-card FSRS updates still work; aggregate stats stay global.
  ...
  if (category) {
    router.setTitle(`مرور: ${category.title}`);
  } else if (multiCategoryIds && multiCategoryIds.length > 1) {
    router.setTitle(`مرور ${multiCategoryIds.length.toLocaleString('fa-IR')} دسته`);
  } else {
    router.setTitle('مرور روزانه');
  }
  ...
  function renderCurrentCard() {
    ...
    bookmarkBtn.innerHTML = `<span class="material-symbols-rounded bookmark-icon">${currentCard.bookmark ? 'bookmark_fill' : 'bookmark'}</span>`;
    bookmarkBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      currentCard.bookmark = !currentCard.bookmark;
      bookmarkBtn.querySelector('.bookmark-icon').textContent = currentCard.bookmark ? 'bookmark_fill' : 'bookmark';
      bookmarkBtn.setAttribute('aria-label', currentCard.bookmark ? 'حذف نشانه' : 'نشانه گذاری');
    ...
    async function handleRating(rating) {
      ...
      try {
        ...
        rescheduleDueCardsReminder().catch(() => {});
        refreshOngoingStatusNotification().catch(() => {});
