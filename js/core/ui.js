...
export const LATEX_SYMBOL_MAP = [
  ...
  { regex: /\\overset\s*\{([^{}]*)\}\s*\{([^{}]*)\}/g, replacement: '<span style="display:inline-flex; flex-direction:column-reverse; align-items:center; vertical-align:middle; line-height:1.1;"><span>$2</span><span style="font-size:0.65em;">$1</span></span>' },
  { regex: /\\cancelto\s*\{([^{}]*)\}\s*\{([^{}]*)\}/g, replacement: '<span style="position:relative; display:inline-block; padding:3px 4px 1px 4px; margin:0 1px;"><span style="position:absolute; top:-0.95em; left:50%; transform:translateX(-50%); font-size:0.62em; color:var(--color-primary); font-weight:700; white-space:nowrap;">$1</span><span style="background:linear-gradient(to top right, transparent calc(50% - 1.1px), currentColor calc(50% - 1.1px), currentColor calc(50% + 1.1px), transparent calc(50% + 1.1px)); display:inline-block;">$2</span></span>' },
  { regex: /\\cancel\s*\{([^{}]*)\}/g, replacement: '<span style="background:linear-gradient(to top right, transparent calc(50% - 1.1px), currentColor calc(50% - 1.1px), currentColor calc(50% + 1.1px), transparent calc(50% + 1.1px)); display:inline-block; padding:1px 3px; margin:0 1px;">$1</span>' },
  { regex: /\\underset\s*\{([^{}]*)\}\s*\{([^{}]*)\}/g, replacement: '<span style="display:inline-flex; flex-direction:column; align-items:center; vertical-align:middle; line-height:1.1;"><span>$2</span><span style="font-size:0.65em;">$1</span></span>' },
  { regex: /\\pmod\s*\{([^{}]*)\}/g, replacement: ' (mod $1)' },
  { regex: /\\bmod/g, replacement: ' mod ' },
  { regex: /\\dot\s*\{([^{}]*)\}/g, replacement: '$1&#775;' },
  { regex: /\\ddot\s*\{([^{}]*)\}/g, replacement: '$1&#776;' },
  { regex: /\\tilde\s*\{([^{}]*)\}/g, replacement: '$1&#771;' },
  { regex: /\\widetilde\s*\{([^{}]*)\}/g, replacement: '$1&#771;' },
  { regex: /\\widehat\s*\{([^{}]*)\}/g, replacement: '$1&#770;' },
  { regex: /\\check\s*\{([^{}]*)\}/g, replacement: '$1&#780;' },
  { regex: /\\underline\s*\{([^{}]*)\}/g, replacement: '<span style="text-decoration:underline;">$1</span>' },
  { regex: /\\overrightarrow\s*\{([^{}]*)\}/g, replacement: '<span style="text-decoration:overline;">$1</span>&#8407;' },
  { regex: /\\overleftarrow\s*\{([^{}]*)\}/g, replacement: '&#8406;<span style="text-decoration:overline;">$1</span>' },
  { regex: /\\[bB]igg?g?\s*([([{|.])/g, replacement: '$1' },
  { regex: /\\[bB]igg?g?\s*([)\]}|.])/g, replacement: '$1' },
  { regex: /\\limits|\\nolimits/g, replacement: '' },
  { regex: /\\bigcirc/g, replacement: '○' },
  { regex: /\\square|\\Box/g, replacement: '□' },
  { regex: /\\blacksquare/g, replacement: '■' },
  { regex: /\\diamond|\\Diamond/g, replacement: '◇' },
  { regex: /\\star/g, replacement: '⋆' },
  { regex: /\\bigstar/g, replacement: '★' },
  { regex: /\\complement/g, replacement: '∁' },
  { regex: /\\top/g, replacement: '⊤' },
  { regex: /\\bot/g, replacement: '⊥' },
  { regex: /\\dagger/g, replacement: '†' },
  { regex: /\\ddagger/g, replacement: '‡' },
  { regex: /\\aleph/g, replacement: 'ℵ' },
  { regex: /\\hbar/g, replacement: 'ℏ' },
  { regex: /\\ell/g, replacement: 'ℓ' },
  { regex: /\\Re/g, replacement: 'ℜ' },
  { regex: /\\Im/g, replacement: 'ℑ' },
  { regex: /\\wp/g, replacement: '℘' },
  { regex: /\\prime/g, replacement: '′' },
  { regex: /\\bigcup/g, replacement: '⋃' }, { regex: /\\bigcap/g, replacement: '⋂' },
  { regex: /\\bigvee/g, replacement: '⋁' }, { regex: /\\bigwedge/g, replacement: '⋀' },
  { regex: /\\bigoplus/g, replacement: '⊕' }, { regex: /\\bigotimes/g, replacement: '⊗' },
  { regex: /\\biguplus/g, replacement: '⊎' }, { regex: /\\coprod/g, replacement: '∐' },
  { regex: /\\iiint/g, replacement: '∭' }, { regex: /\\iint/g, replacement: '∬' },
  { regex: /\\wedge|\\land/g, replacement: '∧' }, { regex: /\\vee|\\lor/g, replacement: '∨' },
  { regex: /\\oplus/g, replacement: '⊕' }, { regex: /\\otimes/g, replacement: '⊗' },
  { regex: /\\ominus/g, replacement: '⊖' }, { regex: /\\oslash/g, replacement: '⊘' },
  { regex: /\\odot/g, replacement: '⊙' }, { regex: /\\uplus/g, replacement: '⊎' },
  { regex: /\\ni\b/g, replacement: ' ∋ ' },
  { regex: /\\models/g, replacement: '⊨' },
  { regex: /\\vdash/g, replacement: '⊢' }, { regex: /\\dashv/g, replacement: '⊣' },
  { regex: /\\doteq/g, replacement: '≐' }, { regex: /\\triangleq/g, replacement: '≜' },
  { regex: /\\smile/g, replacement: '⌣' }, { regex: /\\frown/g, replacement: '⌢' },
  { regex: /\\asymp/g, replacement: '≍' }, { regex: /\\bowtie/g, replacement: '⋈' },
  { regex: /\\simeq/g, replacement: '≃' }, { regex: /\\wr/g, replacement: '≀' },
...
export function renderMathSegment(mathText) {
  ...
  out = out.replace(/\\begin\{cases\}([\s\S]*?)\\end\{cases\}/g, (m, body) => {
    const rows = body.split('\\\\').map((r) => r.trim()).filter(Boolean);
    const rowsHtml = rows.map((row) => {
      const parts = row.split('&');
      const expr = renderMathSegment((parts[0] || '').trim());
      const cond = parts[1] ? renderMathSegment(parts[1].trim()) : '';
      return `<div style="display:flex; gap:10px; white-space:nowrap;"><span>${expr}</span>${cond ? `<span style="color:var(--text-secondary); font-size:0.85em;">${cond}</span>` : ''}</div>`;
    }).join('');
    return `<span style="display:inline-flex; align-items:center; vertical-align:middle;"><span style="font-size:1.8em; margin-left:4px; line-height:1; font-family:serif;">&#123;</span><span style="display:inline-flex; flex-direction:column; gap:2px; text-align:right;">${rowsHtml}</span></span>`;
  });
  ...
  out = out.replace(/\\begin\{(matrix|pmatrix|bmatrix|vmatrix|Bmatrix)\}([\s\S]*?)\\end\{\1\}/g, (m, kind, body) => {
    const [open, close] = MATRIX_BRACKETS[kind];
    const rows = body.split('\\\\').map((r) => r.trim()).filter(Boolean);
    const rowsHtml = rows.map((row) => {
      const cells = row.split('&').map((c) => `<span style="padding:0 6px;">${renderMathSegment(c.trim())}</span>`).join('');
      return `<div style="display:flex; justify-content:center; white-space:nowrap;">${cells}</div>`;
    }).join('');
    return `<span style="display:inline-flex; align-items:center; vertical-align:middle; gap:2px;">${open ? `<span style="font-size:1.6em; line-height:1;">${open}</span>` : ''}<span style="display:inline-flex; flex-direction:column;">${rowsHtml}</span>${close ? `<span style="font-size:1.6em; line-height:1;">${close}</span>` : ''}</span>`;
  });
