...
export const db = {
  getDb() {
    ...
    return new Promise((resolve, reject) => {
      const request = indexedDB.open('personal-learning-os-db', DB_VERSION);
      ...
      request.onupgradeneeded = (e) => {
        ...
        // v2: Weekly Study Planner — a persistent, user-customizable list of
        // subject/activity rows, plus one record per ISO-ish (Saturday-start)
        // week holding that week's cell values.
        if (!d.objectStoreNames.contains('planner_subjects')) {
          const subjStore = d.createObjectStore('planner_subjects', { keyPath: 'id' });
          subjStore.createIndex('order', 'order', { unique: false });
        }
        if (!d.objectStoreNames.contains('planner_weeks')) {
          // keyPath is the week's Saturday date string (YYYY-MM-DD) — one record per week.
          d.createObjectStore('planner_weeks', { keyPath: 'weekStart' });
        }
      };
    });
  },
