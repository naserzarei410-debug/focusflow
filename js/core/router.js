...
let navDirection = 'tab'; // 'tab' | 'push' | 'pop' — decides which entrance animation the next render() uses
...
const PAGE_TITLES = {
  ...
  planner: 'برنامه‌ریزی هفتگی',
};

const SECONDARY_ROUTES = new Set(['category', 'study', 'practice', 'exam', 'search', 'pomodoro', 'calculator', 'planner']);
const BACK_TARGETS = { category: 'library', study: 'library', practice: 'library', exam: 'library', search: 'home', pomodoro: 'home', calculator: 'home', planner: 'home' };
...
function navigate(routeName, param = null) {
  ...
  // Any direct navigate() call is a "push" (going deeper) unless goBack()
  // just set 'pop' right before calling this — used to pick the entrance
  // animation in render() below.
  if (navDirection !== 'pop') {
    navDirection = SECONDARY_ROUTES.has(routeName) ? 'push' : 'tab';
  }
...
function render(routeName, param) {
  ...
  content.innerHTML = '';
  content.classList.remove('page-fade', 'page-push-in', 'page-pop-in');
  void content.offsetWidth;
  const entranceClass = navDirection === 'push' ? 'page-push-in'
                       : navDirection === 'pop'  ? 'page-pop-in'
                       : 'page-fade';
  content.classList.add(entranceClass);
  navDirection = 'tab'; // consumed — back to the default until the next navigate()/goBack()
