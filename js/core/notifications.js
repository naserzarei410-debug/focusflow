...
import { getMostProductiveHour } from './study.js';

/** Varied start-reminder bodies — reduces banner blindness. */
const START_REMINDER_MESSAGES = [
  'الان زمان «{title}» است. شروع کنید.',
  'یه فرصت کوچیک برای «{title}» — همین الان.',
  '۵ دقیقه کافیه تا «{title}» رو شروع کنی.',
  'ذهنت آماده‌ست؟ وقت «{title}» رسیده.',
  'یک قدم کوچک: همین الان «{title}» را شروع کن.',
  'زمان «{title}» فرا رسیده — شروع کن و ادامه بده.',
];

/** Varied end-reminder bodies. */
const END_REMINDER_MESSAGES = [
  'بازه «{title}» به پایان رسید. اگر انجام نداده‌اید، الان فرصت خوبی است.',
  'پایان بازه «{title}». هنوز می‌تونی جبران کنی.',
  'بازه «{title}» تموم شد — اگه مونده، چند دقیقه کافیه.',
  'یادت نره: بازه «{title}» به پایان رسید.',
];

function pickMessage(templates, title) {
  const t = title || 'یادآوری';
  const msg = templates[Math.floor(Math.random() * templates.length)] || templates[0];
  return msg.replace(/\{title\}/g, t);
}
...
export async function rescheduleAllPlanNotifications() {
  ...
  const smartHourOn = (await db.getSetting('notif_smart_hour_enabled', '0')) !== '0';
  let smartHour = null;
  if (smartHourOn) {
    try {
      smartHour = await getMostProductiveHour();
    } catch (e) {
      smartHour = null;
    }
  }
  ...
  for (const plan of plans) {
    ...
    if (plan.notifyStart !== false) {
      const startHour =
        smartHour != null ? smartHour : (Number(plan.startHour) || 0);
      const startMinute =
        smartHour != null ? 0 : (Number(plan.startMinute) || 0);
      const at = nextOccurrence(startHour, startMinute);
      notifications.push({
        id: NotifIds.planBase + slot,
        title: 'زمان شروع: ' + title,
        body: pickMessage(START_REMINDER_MESSAGES, title),
        channelId: CHANNEL_ID,
        schedule: { at, allowWhileIdle: true },
        extra: { route: 'home', planId: plan.id, kind: 'start', smart: smartHour != null },
      });
      slot += 1;
    }
    if (plan.notifyEnd) {
      // End time stays manual — smart hour only shifts the start cue.
      ...
      notifications.push({
        ...
        body: pickMessage(END_REMINDER_MESSAGES, title),
        channelId: CHANNEL_ID,
        schedule: { at, allowWhileIdle: true },
