 ...
 */
export async function chatWithAI({ message, history, systemInstruction, attachments, signal }) {
  const providerId = await getActiveProvider();
  const { apiKey, model } = await getProviderCredentials(providerId);
  ...
  if (providerId === 'groq') {
    return chatWithGroq({ apiKey, model, message, history, systemInstruction, attachments, signal });
  }
  if (providerId === 'openrouter') {
    return chatWithOpenRouter({ apiKey, model, message, history, systemInstruction, attachments, signal });
  }
  if (providerId === 'deepseek') {
    return chatWithDeepSeek({ apiKey, model, message, history, systemInstruction, attachments, signal });
  }
  return chatWithGemini({ apiKey, model, message, history, systemInstruction, attachments, signal });
}
...
export async function generateCardsWithAI({ text, categoryTitle }) {
  ...
  if (providerId === 'deepseek') {
    return generateCardsWithDeepSeek({ apiKey, model, text, categoryTitle, systemInstruction: FLASHCARD_SYSTEM_INSTRUCTION });
  }
