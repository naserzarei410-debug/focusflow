...
/** Fast, synchronous cache so the *next* app launch can paint the right
 *  theme immediately (see the inline script at the top of index.html) —
 *  IndexedDB (db.js) can't be read synchronously, so without this the
 *  app would always flash the light theme for a moment before its
 *  async initTheme() below finishes resolving the real saved setting. */
function cacheThemeModeForNextLaunch(mode) {
  try { localStorage.setItem('theme_mode_cache', mode); } catch (e) { /* storage unavailable — non-fatal */ }
}

export const theme = {
  async initTheme() {
    const mode = await db.getSetting('theme_mode', 'auto');
    cacheThemeModeForNextLaunch(mode);
  ...
  async toggleTheme() {
    ...
    cacheThemeModeForNextLaunch(next);
  ...
  async getThemeMode() {
    return db.getSetting('theme_mode', 'auto');
  },
