...
export function schedule(card, rating, reviewDate = new Date()) {
  ...
  if (card.lastReviewed) {
    ...
    if (!isNaN(lastRev.getTime())) {
      // Fractional days (not floored to whole days) — reviewing a card
      // again a few minutes/hours after the last review must still count
      // as "some time passed", or retrievability below always comes out
      // to exactly 1.0 and every rating (Hard/Good/Easy) ends up
      // producing the identical next interval (see calculateRetrievability).
      const diffTime = Math.max(0, reviewDate.getTime() - lastRev.getTime());
      elapsedDays = diffTime / (1000 * 60 * 60 * 24);
    }
  }
  if (isNaN(elapsedDays) || !isFinite(elapsedDays)) elapsedDays = 0;
  ...
  if (previousState === State.New) {
    ...
    // Only "Again" is a genuine failure (forgot). "Hard" is still a correct,
    // successful recall — just a slower/less confident one — so it must be
    // treated as a success like Good/Easy and allowed to graduate to the
    // long-term Review state. Treating Hard the same as Again here used to
    // trap a card in this short-term loop forever, since the only ratings
    // that could ever move it to State.Review were Good/Easy.
    if (rating === Rating.Again) {
      nextStability = previousStability * 0.5;
      nextDifficulty = Math.min(previousDifficulty + 1.0, 10.0);
      nextState = previousState;
    } else {
      // Hard still succeeds, but more cautiously than Good; Easy grows the most.
      const growthFactor = rating === Rating.Hard ? 1.2 : (rating === Rating.Easy ? 2.0 : 1.5);
      nextStability = previousStability * growthFactor;
    ...
    // The stability formulas must use the difficulty as it was BEFORE this
    // review (previousDifficulty), not the value already updated for this
    // review. Using the post-update difficulty here was double-applying the
    // effect of the rating and skewed every resulting interval.
    if (rating === Rating.Again) {
      nextStability = updateStabilityFailure(previousStability, previousDifficulty, r);
      nextState = State.Relearning;
    } else {
      nextStability = updateStabilitySuccess(previousStability, previousDifficulty, r, rating);
      nextState = State.Review;
    }
    nextDifficulty = updateDifficulty(previousDifficulty, rating);
  }
